...
for(const src of ['app.js','patch.js','mobile-app.js','game-day-checklist.js','season-planning.js','netball-operations.js','mobile-open-games.js','simple-ux.js'])assert(html.includes(`<script src="${src}"></script>`),`index.html does not load ${src}`);
assert(!checklist.includes('createElement(\'script\')'),'Checklist still dynamically loads scripts');
assert(html.indexOf('season-planning.js')<html.indexOf('netball-operations.js')&&html.indexOf('netball-operations.js')<html.indexOf('mobile-open-games.js'),'Module load order is not deterministic');
...
assert(app.includes("['SETTINGS',['import'"),'Season Import is not positioned under Settings in the base app');
...
assert(checklist.includes("disabled"),'Checklist sign-off is not gated by important controls');
...
console.log(`ClubRoster static QA passed: ${requiredPages.length} substantive screens, fixed module loading, mobile/checklist controls, forward roster horizons, advance availability and controlled umpire swaps.`);
