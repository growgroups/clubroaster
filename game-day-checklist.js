const coordinatorChecklist={
  ...
  sections:[
    ...
    {id:'during',title:'During play',items:[
      {id:'coverage',label:'Monitor court coverage, breaks, withdrawals and replacement To Do items',done:false,critical:true},
    ...
    {id:'close',title:'End of day',items:[
      ...
      {id:'openincidents',label:'Confirm all incidents have an owner, status and follow-up item',done:false,critical:true},
...
function coordinatorChecklistHtml(){return `<div class="appHero"><small>${coordinatorChecklist.round} · ${coordinatorChecklist.date}</small><strong>Game Day Checklist</strong><small>${checklistPct()}% complete · ${checklistCriticalOutstanding().length} important checks left</small></div><div class="appCard"><div class="toolbar" style="justify-content:space-between"><div><h3>Coordinator checklist</h3><div class="appMeta">Work down this list to keep the day safe and organised.</div></div>${tag(checklistCriticalOutstanding().length+' important',checklistCriticalOutstanding().length?'red':'green')}</div>${progress(checklistPct())}</div>${coordinatorChecklist.sections.map(s=>`<div class="appCard"><h3>${esc(s.title)}</h3>${s.items.map(i=>`<label class="checkrow"><input type="checkbox" data-checklist-id="${i.id}" ${i.done?'checked':''}><div><b>${esc(i.label)}</b><p class="sub">${i.critical?'Important check':'Normal check'} · ${i.done?'Done':'Still to do'}</p></div></label>`).join('')}</div>`).join('')}<div class="appCard"><h3>Finish the day</h3><div class="appMeta">You can sign off only when all important checks are done.</div><div class="appActions"><button class="btn primary" data-checklist-action="signoff" ${checklistCriticalOutstanding().length?'disabled':''}>${coordinatorChecklist.signedOff?'Day signed off':'Sign off game day'}</button><button class="btn" data-checklist-action="tasks">Add unfinished items to To Do</button></div>${coordinatorChecklist.signedOff?'<div class="success" style="margin-top:10px">Game day signed off by Umpire Coordinator · 22 Aug 2026.</div>':''}</div>`}
function openCoordinatorChecklist(){openDrawer('Game Day Coordinator Checklist',coordinatorChecklistHtml())}
function syncChecklistTask(item){const title='Checklist: '+item.label;let t=tasks.find(x=>x.title===title);if(!item.done){if(!t)tasks.push({id:tasks.length+1,type:'Game Day',priority:item.critical?'High':'Medium',title,detail:'Unfinished game-day checklist item.',from:'Coordinator Checklist',status:'Open',action:'gameday',due:'Today'})}else if(t)t.status='Resolved'}
...
document.addEventListener('click',e=>{const a=e.target.closest('[data-checklist-action]');if(!a)return;if(a.dataset.checklistAction==='tasks'){checklistOutstanding().forEach(syncChecklistTask);toast('Unfinished checklist items added to To Do.');return}if(a.dataset.checklistAction==='signoff'){if(checklistCriticalOutstanding().length){toast('Finish all important checks before signing off.');return}coordinatorChecklist.signedOff=true;auditEvents.unshift(['16 Aug 5:05pm','Game Day','Coordinator day sign-off','Game-day checklist completed and signed off']);openDrawer('Game Day Coordinator Checklist',coordinatorChecklistHtml());toast('Game day signed off.')}});
const originalCoordinatorHome=coordinatorHome;
coordinatorHome=function(){return originalCoordinatorHome()+`<div class="appCard" style="margin-top:10px"><div class="toolbar" style="justify-content:space-between"><div><h3>Game Day Checklist</h3><div class="appMeta">${checklistPct()}% done · ${checklistOutstanding().length} left · ${checklistCriticalOutstanding().length} important</div></div>${tag(checklistCriticalOutstanding().length+' important',checklistCriticalOutstanding().length?'red':'green')}</div>${progress(checklistPct())}<div class="appActions"><button class="btn primary" data-mobile-action="coordinatorChecklist">Open checklist</button></div></div>`};
